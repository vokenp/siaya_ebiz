    ...
    $LogChanges = $auditLog[1];
     ...
     if (empty($arg)) {
       ...
       $db->AutoExecute($table,$record,$action,$criteria);
        ...
        if ($db) {
          if ($table == "tbl_teachers") {
              if (isset($_POST['Department'])) {
                $Dept = explode(',', $record['Department']);
                $exec = $db->Execute("delete from listitems where ItemType='TeacherCourses' and ItemCode='$S_ROWID'");
                foreach ($Dept as $key => $dval) {
                  $rec["ItemDescription"] = $dval;
                  $rec["ItemCode"] = $S_ROWID;
                  $rec["ItemType"] = "TeacherCourses";
                  $rec["CreatedBy"] = $user;
                  $table  = "listitems";
                  $action = "INSERT";
