...
if (move_uploaded_file($userfile_tmp, $large_image_location)){
		...
		//do your saving to db here the path is $large_image_location
          ...
          $record["FileDescription"] = $_POST["DocumentTitle"];
          ...
          $Descrip = $_POST["DocumentTitle"];
            ...
            $rec["DocID"] = $docID;
            $rec["DocumentDate"] = date('Y-m-d',strtotime($_POST["DocumentDate"])) ;
            $rec["DocumentTitle"] = $_POST["DocumentTitle"];
            $rec["DocumentCategory"] = $StoragePool;
              if (isset($_POST['CommitteeDocType'])) {
                $rec["CommitteeDocType"] = safehtml($_POST['CommitteeDocType']);
              }
            $rec["CreatedBy"] = $user;
            $rec["DateCreated"] = $db->GetOne("select current_timestamp");

            $table ="assemblybizdocs";
            $action = "INSERT";
