  ...
  $method = isset($_POST["Method"]) ? $_POST["Method"] : "";
   if ($method == "getDocCategories") {
      ...
      while (!$getDocCategories->EOF) {
        ...
        $DocCount = $db->GetOne("select count(*) from assemblybizdocs where DocumentCategory='$DocCatID'");
      	  if ($DocCount != 0) {
            $iconRand = rand(1,8);
            $iconPath = "/assets/StoragePool/icons/icon$iconRand.png";
   ...
   if ($method == "getNewNotifications") {
      ...
      while (!$getNots->EOF) {
        ...
        $NSubject = $rst["NType"]." posted on ".date('D jS M Y g:i a',strtotime($rst["DateCreated"]));
      ...
      $MemID = safehtml($_POST['MemID']);
      $getNots =  $db->Execute("select *from notifications where NStatus='Read' and NTargetedTo='$MemID'");
      $response = array();
  ...
  if ($method == "getReminders") {
    ...
    if ($method == "UpdateNotification") {
     ...
     if ($method == "DeleteNotification") {
        $NotificationID = safehtml($_POST['NotificationID']);
        $tbl = "notifications";
        $NotInfo = $rs->row($tbl,"S_ROWID='$NotificationID'");
        $user = $NotInfo["NTargetedTo"];
        logAction($NotificationID,$tbl,$user,"Delete","",999);
        //$exec = $db->Execute("update notifications set NStatus='Read',DateModified=current_timestamp where S_ROWID='$NotificationID'");
    }
    ...
    if ($method == "getSummaryReport") {
      ...
      $rptMonth = safehtml($_POST['rptMonth']);
      $MemID = safehtml($_POST['MemID']);
...
foreach ($MonthWeek as $key => $weekVals) {
  $key += 1;
    ...
    $Payment = array();
    $Payment["TotalMeetings"] = rand(1,4);
    $Payment["ExtraMeetings"] = 1;
    $Payment["GrossPay"] = $allawancesRate[$AlloIndex] * $Payment["TotalMeetings"];
     $Payment["AmountPayable"] = $Payment["GrossPay"] + ($allawancesRate[$ExtraIndex] * $Payment["ExtraMeetings"]);
  ...
  $Wlist["GrandTotal"] = $grandTotal;
   $Wlist["ReportPath"] = "/assets/StoragePool/tmpreports/test.pdf";
...
$allawancesRate = array(6500,5200,3900);
  ...
  $days2Show = array("Monday","Tuesday","Wednesday","Thursday");
