...
$getCols = $db->GetArray("select FieldName,DisplayName from dh_listview where ModuleCode='$ModuleCode'  and ListType='Main' and TableName='$TableName' order by DisplayOrder asc");
  ...
  function getUserAction($userID,$ModCode)
  {
    global $db;
    $UserProfiles = $this->UserProfile($userID);
...
{
  global $db;
  $getdata = $db->Execute("select distinct ItemDescription as ProfileID,dp.ProfileName from listitems l inner join dh_userprofiles dp on l.ItemDescription=dp.S_ROWID where ItemType='RoleProfile' and ItemCode in (select ItemCode from listitems where ItemType='RoleUser' and ItemDescription='$UserID') ");
   $list = array();
  while (!$getdata->EOF) {
